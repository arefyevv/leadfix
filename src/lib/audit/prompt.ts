...
export function buildAuditPrompt(analysis: AuditAnalysis) {
  ...
  const user = [
    ...
    [
      ...
      "Для каждого issue во всех 8 категориях поля problem, evidence, recommendation, expectedResult и example должны быть короткими: 1-2 предложения. problem отвечает 'что не так', evidence - 'почему мешает', recommendation - 'что исправить', expectedResult - 'что изменится', example - готовая примерная формулировка.",
