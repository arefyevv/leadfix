...
function createPreviewReport(data: Omit<AuditAnalysis, "previewReport">): PreviewReport {
  ...
  if (data.h1.length === 0) {
    addInsight(15, {
      title: "На странице нет заголовка H1",
      description: "Главный оффер не выделен семантически. Добавьте один понятный H1 с ключевой выгодой для клиента.",
      priority: "Критично"
    });
  }
  ...
  if (data.ctaSignals.length === 0) {
    addInsight(20, {
      ...
      description: "На странице нет CTA с ясным следующим шагом. Добавьте заметную кнопку: заказать, получить расчёт или оставить заявку.",
      priority: "Критично"
    });
  }
  ...
  if (data.title.length < 30) {
    addInsight(5, {
      title: data.title ? "Title слишком короткий" : "На странице нет title",
      description: "Раскройте в title услугу и основную ценность страницы. Это улучшит понятность страницы и сниппет в поиске.",
      priority: "Низкий"
    });
  }
  ...
  if (!data.description) {
    addInsight(5, {
      title: "Нет meta description",
      description: "Добавьте краткое описание предложения страницы для более понятного поискового сниппета.",
      priority: "Низкий"
    });
  }
